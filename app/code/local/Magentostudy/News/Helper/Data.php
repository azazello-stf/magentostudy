...
class Magentostudy_News_Helper_Data
    extends Mage_Core_Helper_Data
{
    ...
    /**
     * Path to store config where count of newsposts per page is stored
     *
     * @var string
     */
    ...
    /**
     * NewsItem instance for lazy loading
     *
     ...
     * @return int
     */
    public function getNewsPerPage($store)
    {
    ...
    {
        if(!$this->_newsItemInsatnce){
            $this->_newsItemInstance = Mage::registry('news_item');

            if(!$this->_newsItemInsatnce){
                Mage::throwException($this->__('New item instance does not exist in Registry'));
            }
        }
